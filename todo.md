...
- [ ] תיקון רקע UploadReport - תמונת השער (8K Resolution / High Fi / FINAN..) גלויה ומתחרה עם תיבת ההעלאה - להסיר/להחליף ברקע סינמטי שקט
- [ ] הוספת מצב מצגת לדמו (`/demo?clean=true` או דומה) - מסתיר כפתור "חזרה למסך הראשי" וכל ה-shell, נשאר רק הדמו במסך מלא
- [ ] שיפור הבהירות של הניווט בין /dashboard לדמו - הסבר ברור איפה "המסך הראשי" של המערכת (Banner / Tooltip / Onboarding hint)
- [ ] בדיקה ויזואלית של התוצאה הסופית עם screenshots לכל המסכים שתוקנו
