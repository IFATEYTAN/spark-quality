...
- [ ] הוספת קישור בולט "צפייה בדמו ההדרכה" ב-Header של CinematicShell (גם ב-Landing וגם ב-Onboarding)
- [ ] בדיקה ויזואלית מלאה של זרימת `/demo` (Splash → Intro → Upload → Analyzing → Dashboard → Actions → Summary)
- [ ] screenshots מסודרים של כל שלבי הדמו עם הקטגוריות הפיננסיות
